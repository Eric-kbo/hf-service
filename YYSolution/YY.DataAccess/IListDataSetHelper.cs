...
namespace YY.DataAccess
{

    /// <summary>
    /// list与DataSet的相互转换
    /// </summary>
    public class IListDataSetHelper
    {
        /// <summary> 
        /// 集合装换DataSet 
        /// </summary> 
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    for (int i = 0; i < p_Data.Columns.Count; i++)
                    {
                        ...
                        if (pi.Name.Equals(p_Data.Columns[i].ColumnName))
                        {
                            ...
                            if (p_Data.Rows[j][i] != DBNull.Value)
                            {
                                pi.SetValue(_t, DataChange(p_Data.Rows[j][i], pi), null);
                            }
                            else
                            {
                                pi.SetValue(_t, null, null);
        ...
        private static object DataChange(object p, PropertyInfo pi)
        {
            object resule;
            switch (pi.PropertyType.Name)
            {
                case "String":
                    ...
                    break;
                case "Int":
                    resule = Convert.ToInt32(p);
                    break;
                case "Long":
                    resule = Convert.ToInt64(p);
                    break;
                case "Short":
                    resule = Convert.ToInt16(p);
                    break;
                case "Decmail":
                    resule = Convert.ToDecimal(p);
                    break;
                case "Double":
                    resule = Convert.ToDouble(p);
                    break;
                case "Bool":
                    resule = Convert.ToBoolean(p);
                    break;
                case "Datetime":
                case "Date":
                    resule = Convert.ToDateTime(p);
                    break;
                case "Char":
                    resule = Convert.ToChar(p);
                    break;
                default:
                    resule = p;
                    break;
